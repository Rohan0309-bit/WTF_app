...
import { basketballSubCategories, type BasketballDrill } from '@/lib/basketball-drills';
...
function BasketballDrillsContent() {
        ...
        {selectedCategory && drillsForCategory.length > 0 ? (
            <DrillsLibrary drills={drillsForCategory} categoryName={selectedCategory} />
             ...
             <Card className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-8 border-dashed">
                ...
                <p className="text-muted-foreground mb-4">Please go back to the Basketball Hub to choose a drill category.</p>
                <Link href="/dashboard/general-workouts/athlete/basketball">
                    <Button variant="outline">
                        <ChevronLeft className="mr-2 h-4 w-4" />
                        Back to Basketball Hub
                    </Button>
                </Link>
             </Card>
        )}
...
export default function BasketballDrillsPage() {
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <BasketballDrillsContent />
        </Suspense>
    )
}
