...
import {
  FOOTBALL_MALE_GYM_WORKOUT_PLAN,
  FOOTBALL_MALE_HOME_WORKOUT_PLAN,
  FOOTBALL_FEMALE_GYM_WORKOUT_PLAN,
  FOOTBALL_FEMALE_HOME_WORKOUT_PLAN,
  WorkoutSplit
} from '@/lib/workouts';
...
export default function FootballWorkoutPage() {
  const [gender, setGender] = useState<Gender>('male');
  const [location, setLocation] = useState<Location>('gym');
  const [selectedDay, setSelectedDay] = useState<string>('Monday');

  const getWorkoutPlan = (): WorkoutSplit => {
    if (gender === 'male' && location === 'gym') return FOOTBALL_MALE_GYM_WORKOUT_PLAN;
    if (gender === 'male' && location === 'home') return FOOTBALL_MALE_HOME_WORKOUT_PLAN;
    if (gender === 'female' && location === 'gym') return FOOTBALL_FEMALE_GYM_WORKOUT_PLAN;
    return FOOTBALL_FEMALE_HOME_WORKOUT_PLAN;
  };
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold font-headline">Football Workout</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Sport-specific program for Football players to improve power, agility, and on-field performance.
        </p>
      </div>
