...
import { footballSubCategories, type FootballDrill } from '@/lib/football-drills';
...
function DrillsLibrary({ drills, categoryName }: { drills: FootballDrill[], categoryName: string }) {
...
function FootballDrillsContent() {
  const searchParams = useSearchParams();
  const selectedCategory = searchParams.get('category');
  
  const drillsForCategory = useMemo(() => {
    if (!selectedCategory) return [];
    return footballSubCategories[selectedCategory] || [];
  }, [selectedCategory]);
        ...
        {selectedCategory && drillsForCategory.length > 0 ? (
            <DrillsLibrary drills={drillsForCategory} categoryName={selectedCategory} />
             ...
             <Card className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-8 border-dashed">
                ...
                <p className="text-muted-foreground mb-4">Please go back to the Football Hub to choose a drill category.</p>
                <Link href="/dashboard/general-workouts/athlete/football">
...
export default function FootballDrillsPage() {
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <FootballDrillsContent />
        </Suspense>
    )
}
