...
import { volleyballSubCategories, type VolleyballDrill } from '@/lib/volleyball-drills';
...
function DrillsLibrary({ drills, categoryName }: { drills: VolleyballDrill[], categoryName: string }) {
  const [selectedDrill, setSelectedDrill] = useState<VolleyballDrill | null>(null);
...
function VolleyballDrillsContent() {
  const searchParams = useSearchParams();
  const selectedCategory = searchParams.get('category');
  
  const drillsForCategory = useMemo(() => {
    if (!selectedCategory) return [];
    return volleyballSubCategories[selectedCategory] || [];
  }, [selectedCategory]);

  return (
    <div className="container mx-auto p-4">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold font-headline">{selectedCategory || 'Volleyball Drills Library'}</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          {selectedCategory 
            ? `Hone your skills with our expert collection of ${selectedCategory}.`
            : "Select a category from the Volleyball Hub to view drills."
          }
        </p>
      </div>
        ...
        {selectedCategory && drillsForCategory.length > 0 ? (
            <DrillsLibrary drills={drillsForCategory} categoryName={selectedCategory} />
             ...
             <Card className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-8 border-dashed">
                ...
                <p className="text-muted-foreground mb-4">Please go back to the Volleyball Hub to choose a drill category.</p>
                <Link href="/dashboard/general-workouts/athlete/volleyball">
                    <Button variant="outline">
                        <ChevronLeft className="mr-2 h-4 w-4" />
                        Back to Volleyball Hub
                    </Button>
                </Link>
             </Card>
        )}
...
export default function VolleyballDrillsPage() {
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <VolleyballDrillsContent />
        </Suspense>
    )
}
