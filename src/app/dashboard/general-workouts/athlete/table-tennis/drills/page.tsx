...
import { tableTennisSubCategories, type TableTennisDrill } from '@/lib/table-tennis-drills';
...
function DrillsLibrary({ drills, categoryName }: { drills: TableTennisDrill[], categoryName: string }) {
  const [selectedDrill, setSelectedDrill] = useState<TableTennisDrill | null>(null);
...
function TableTennisDrillsContent() {
  const searchParams = useSearchParams();
  const selectedCategory = searchParams.get('category');
  
  const drillsForCategory = useMemo(() => {
    if (!selectedCategory) return [];
    return tableTennisSubCategories[selectedCategory] || [];
  }, [selectedCategory]);

  return (
    <div className="container mx-auto p-4">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold font-headline">{selectedCategory || 'Table Tennis Drills Library'}</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          {selectedCategory 
            ? `Hone your skills with our expert collection of ${selectedCategory}.`
            : "Select a category from the Table Tennis Hub to view drills."
          }
        </p>
      </div>
        ...
        {selectedCategory && drillsForCategory.length > 0 ? (
            <DrillsLibrary drills={drillsForCategory} categoryName={selectedCategory} />
             ...
             <Card className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-8 border-dashed">
                ...
                <p className="text-muted-foreground mb-4">Please go back to the Table Tennis Hub to choose a drill category.</p>
                <Link href="/dashboard/general-workouts/athlete/table-tennis">
                    <Button variant="outline">
                        <ChevronLeft className="mr-2 h-4 w-4" />
                        Back to Table Tennis Hub
                    </Button>
                </Link>
             </Card>
        )}
...
export default function TableTennisDrillsPage() {
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <TableTennisDrillsContent />
        </Suspense>
    )
}
