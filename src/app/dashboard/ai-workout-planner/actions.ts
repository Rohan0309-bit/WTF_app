...
Task:
- Generate a 7-day workout plan based on the user's details.
...
export async function getWorkoutPlan(
  ...
  try {
    ...
    if (!response.ok) {
        ...
        const errorMessage = errorData.error?.message || 'The AI failed to generate a response. Please check your API key and network connection.';
        return {
    ...
    // Defensive checking of the API response structure
    if (!data.candidates || !data.candidates[0] || !data.candidates[0].content || !data.candidates[0].content.parts || !data.candidates[0].content.parts[0]) {
      console.error("AI response was in an unexpected format:", data);
      return { message: "AI returned an empty or invalid response structure.", isSuccess: false };
    }

    const aiText = data.candidates[0].content.parts[0].text;

    if (!aiText) {
        console.error("AI response text was empty.");
        return { message: "AI returned an empty response.", isSuccess: false };
    }
    ...
    if (!workoutPlanData || !workoutPlanData.workoutPlan || !Array.isArray(workoutPlanData.workoutPlan)) {
      ...
      return { message: "AI returned a workout plan with an invalid structure.", isSuccess: false };
    }
